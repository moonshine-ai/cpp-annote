...
// C++ parity for to_annotation (Binarize onset/offset 0.5 + min_duration_*) vs golden JSON.
...
static bool try_regex_double(const std::string& json, const std::string& key_escaped, double& out) {
...
// Binarize (signal.py) for 2D SlidingWindowFeature: hysteresis on frame middles.
static void binarize_column(
  ...
  for (int i = 0; i < num_frames; ++i) {
    ts[static_cast<size_t>(i)] = sw_start + static_cast<double>(i) * sw_step + 0.5 * sw_duration;
  }
...
static std::vector<CanonicalTurn> parse_diarization_json(const std::string& json) {
  std::vector<CanonicalTurn> rows;
  std::regex block(
      "\\{\\s*\"start\"\\s*:\\s*([-+0-9.eE]+)\\s*,\\s*\"end\"\\s*:\\s*([-+0-9.eE]+)\\s*,"
      "\\s*\"speaker\"\\s*:\\s*\"([^\"]*)\"\\s*\\}");
  for (std::sregex_iterator it(json.begin(), json.end(), block), end; it != end; ++it) {
    CanonicalTurn r;
  ...
  if (rows.empty() && json.find('[') != std::string::npos && json.find(']') != std::string::npos) {
    ...
    while (pos < json.size()) {
      ...
      std::smatch ms, me, mp;
      if (std::regex_search(chunk, ms, re_start) && std::regex_search(chunk, me, re_end) &&
...
int main(int argc, char** argv) {
  if (argc != 2) {
    ...
    std::cerr << "  Needs discrete_diarization_overlap.npz, label_mapping.json, diarization.json,\n";
    std::cerr << "  discrete_diarization_exclusive.npz, exclusive_diarization.json,\n";
    std::cerr << "  and parent ../pipeline_snapshot.json (optional; defaults min_duration_* to 0).\n";
    return 2;
  }
  ...
  try {
    ...
    const std::map<int, std::string> labels = parse_label_mapping(read_text_file(utter + "/label_mapping.json"));

    // Binarize.__call__: support(collar=min_duration_off) only if pad_* or min_duration_off > 0.
    ...
    auto run_pair = [&](const char* discrete_npz, const char* golden_json) {
      ...
      if (arr.shape.size() != 2) {
        throw std::runtime_error("discrete diarization must be rank-2 (frames, speakers)");
      }
      ...
      for (int k = 0; k < num_classes; ++k) {
        ...
        for (int t = 0; t < num_frames; ++t) {
          col[static_cast<size_t>(t)] =
              src[static_cast<size_t>(t) * static_cast<size_t>(num_classes) + static_cast<size_t>(k)];
        }
      ...
      for (const auto& tr : turns) {
        ...
        if (it == labels.end()) {
          throw std::runtime_error("label_mapping missing key for speaker index " + std::to_string(lab));
        }
        got.push_back({std::get<1>(tr), std::get<2>(tr), it->second});
      }

      std::sort(got.begin(), got.end());
      std::vector<CanonicalTurn> gold = parse_diarization_json(read_text_file(utter + "/" + golden_json));
      std::sort(gold.begin(), gold.end());

      if (got.size() != gold.size()) {
        std::ostringstream oss;
        oss << golden_json << ": turn count mismatch got " << got.size() << " gold " << gold.size();
      ...
      for (size_t i = 0; i < got.size(); ++i) {
        if (got[i].speaker != gold[i].speaker) {
          std::ostringstream oss;
          oss << golden_json << " idx " << i << " speaker got \"" << got[i].speaker << "\" gold \"" << gold[i].speaker
              << "\"";
          throw std::runtime_error(oss.str());
        }
        if (std::abs(got[i].start - gold[i].start) > tol || std::abs(got[i].end - gold[i].end) > tol) {
          std::ostringstream oss;
          oss << golden_json << " idx " << i << " bounds got [" << got[i].start << "," << got[i].end << "] gold ["
              << gold[i].start << "," << gold[i].end << "]";
          throw std::runtime_error(oss.str());
        }
      }
    ...
    run_pair("discrete_diarization_exclusive.npz", "exclusive_diarization.json");
