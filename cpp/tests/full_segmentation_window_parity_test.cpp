...
//
// If this fails, oracle ``hard_clusters_final.npz`` (from Python) is mis-matched with
// C++ segmentations and DER will be inflated even when clustering is "oracle".
...
static double json_chunk_step_sec(const std::string& json, double fallback_from_dur) {
...
int main(int argc, char** argv) {
  if (argc != 4) {
    std::cerr << "Usage: full_segmentation_window_parity_test <segmentation.onnx> <golden_utterance_dir> <wav_file>\n";
    std::cerr << "  Compares ORT output for every chunk to golden binarized_segmentations.npz (same layout as diarize).\n";
    return 2;
  }
  try {
  ...
  const double chunk_step_sec = json_chunk_step_sec(json, 0.1 * chunk_dur_sec);
  ...
  int wav_sr = 0;
  std::vector<float> audio_in = wav_pcm::load_wav_pcm16_mono_float32(wav_path, wav_sr);
  std::vector<float> audio = wav_pcm::linear_resample(audio_in, wav_sr, sr_model);
  ...
  std::cout << "audio_samples=" << num_samples << " sr_model=" << sr_model << " total_chunks=" << total_chunks
            << " golden_C=" << C_gold << " F=" << F_gold << " K=" << K_gold << "\n";

  if (total_chunks != C_gold) {
    std::cerr << "FAIL: chunk count mismatch (wav-derived " << total_chunks << " vs golden " << C_gold << ")\n";
    std::cerr << "  Fix: use the same WAV file as dump_diarization_golden / eval (same length and truncation).\n";
    return 1;
  }
  ...
  std::vector<float> seg_out(static_cast<size_t>(total_chunks) * static_cast<size_t>(F_gold) * static_cast<size_t>(K_gold));
  ...
  auto run_chunk = [&](const float* wav_data, int64_t chunk_idx) {
    ...
    if (sh.size() != 3 || sh[0] != 1 || static_cast<int>(sh[1]) != F_gold || static_cast<int>(sh[2]) != K_gold) {
      throw std::runtime_error("unexpected ORT output shape vs golden");
    }
    std::memcpy(
        &seg_out[static_cast<size_t>(chunk_idx) * static_cast<size_t>(F_gold) * static_cast<size_t>(K_gold)],
        op,
        static_cast<size_t>(F_gold * K_gold) * sizeof(float));
  };
  ...
  const double frac_large = static_cast<double>(n_large) / static_cast<double>(n_el);
  const double frac_bin = static_cast<double>(bin_mismatch) / static_cast<double>(n_el);

  std::cout << "max_abs_diff(ORT_seg, golden_bin) = " << mad << "\n";
  std::cout << "fraction(|diff| > 1e-3) = " << frac_large << " (" << n_large << "/" << n_el << ")\n";
  std::cout << "binary_mismatch((a>0.5)!=(b>0.5)) count = " << bin_mismatch << " fraction = " << frac_bin << "\n";
  ...
  if (bad_allclose > 0) {
    ...
    for (size_t i = 0; i < n_el; ++i) {
      ...
      if (!(std::abs(a - b) <= atol + rtol * std::abs(b))) {
        ...
        std::cerr << "  allclose_mismatch[" << printed << "] flat_i=" << i << " chunk=" << c << " frame=" << f
                  << " class=" << k << " ORT=" << a << " golden=" << b << " tol=" << (atol + rtol * std::abs(b))
                  << "\n";
        if (++printed >= 5) {
          break;
        }
  ...
  const bool float_ok = (static_cast<double>(bad_allclose) / static_cast<double>(n_el)) <= 1e-4;
  if (!bin_ok) {
    std::cerr << "FAIL: binary segmentation disagreements exceed 1e-4 of cells — oracle clusters are unsafe.\n";
    return 1;
  }
  if (!float_ok) {
  ...
  if (bad_allclose > 0) {
    std::cout << "PASS (binary parity); NOTE: minor float allclose violations vs golden (" << bad_allclose
              << " cells) — expected at hard 0/1 boundaries under rtol=" << rtol << ", atol=" << atol << ".\n";
  } else {
    std::cout << "PASS full-window segmentation ORT vs golden binarized_segmentations\n";
  }
  return 0;
