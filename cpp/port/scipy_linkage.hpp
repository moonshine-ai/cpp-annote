...
/// Row-major `X`: `n` rows, `d` cols → condensed pairwise Euclidean distances (length n*(n-1)/2).
void pdist_euclidean(const std::vector<double>& X, int n, int d, std::vector<double>& dist);

/// SciPy `linkage(..., method='centroid')` via the generic O(n³) SciPy C implementation (small `n` only).
/// `Z` is written as `(n-1)*4` row-major doubles: for each merge: idx_a, idx_b, dist, count.
void linkage_centroid_naive(const std::vector<double>& dist, int n, std::vector<double>& Z);

/// ``fcluster(Z, t, criterion='distance')`` → labels in **SciPy convention** (1..K inclusive).
/// Implemented like SciPy: ``get_max_dist_for_each_cluster`` + ``cluster_monocrit`` (not pairwise cophenetic union).
