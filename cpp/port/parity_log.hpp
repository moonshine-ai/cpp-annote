...
/// ``PYANNOTE_CPP_PARITY_OUT``: directory for level-2 NPZ bundle (created if missing). Ignored when level < 2.
...
/// FNV-1a 64-bit fingerprint of strided ``float`` samples (must match ``cpp/scripts/parity_fingerprint.py``).
