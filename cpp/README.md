...
- **`pyannote::CppAnnote`** (`src/cpp-annote.h`, `src/cpp-annote.cpp`, embedded defaults in **`src/community1_cpp_annote_embedded.*`**) — same pipeline as **`cpp-annote-cli`**: construct with segmentation ONNX and embedding ONNX; pass empty strings for receptive field, golden bounds, pipeline snapshot, and both PLDA NPZ paths to use compiled-in community-1 defaults, or pass file paths to override. Call **`diarize(std::vector<float> mono, std::int32_t sample_rate)`** to obtain **`std::vector<pyannote::DiarizationTurn>`** (use **`pyannote::write_diarization_json`** to serialize like the CLI). Output speakers are **`SPEAKER_00`**, **`SPEAKER_01`**, … unless you extend the code.
