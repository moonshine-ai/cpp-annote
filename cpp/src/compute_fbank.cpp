...
void wespeaker_like_fbank(
  ...
  if (num_samples > 0) {
    fbank.AcceptWaveform(sample_hz, scaled.data(), static_cast<int32_t>(num_samples));
  }
  fbank.InputFinished();
  ...
  out_rowmajor.assign(static_cast<std::size_t>(num_frames) * static_cast<std::size_t>(mel_dim_out), 0.f);
  for (int t = 0; t < num_frames; ++t) {
    const float* row = fbank.GetFrame(t);
    std::memcpy(
        &out_rowmajor[static_cast<std::size_t>(t) * static_cast<std::size_t>(mel_dim_out)],
        row,
        static_cast<std::size_t>(mel_dim_out) * sizeof(float));
  }
  for (int m = 0; m < mel_dim_out; ++m) {
    ...
    const float mean = static_cast<float>(sum / static_cast<double>(std::max(1, num_frames)));
    for (int t = 0; t < num_frames; ++t) {
      out_rowmajor[static_cast<std::size_t>(t) * static_cast<std::size_t>(mel_dim_out) +
